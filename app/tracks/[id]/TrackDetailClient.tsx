      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="flex-shrink-0 lg:w-1/3">
          <div className="flex flex-col items-center gap-4">
            <Image
              src={resolveTrackAssetSource(release.cover)}
              alt={`${release.title} cover`}
              width={400}
              height={400}
              priority
              sizes="(max-width: 1024px) 100vw, 33vw"
              className="w-full max-w-md h-auto object-contain bg-neutral-950 shadow-lg"
            />
              ...
              {release.tracks.map((track) => {
                ...
                return (
                  <button
                    ...
                    className="w-full text-left px-3 py-2 rounded-lg bg-neutral-800 hover:bg-neutral-700 transition-colors cursor-pointer"
                  >
                    <span className="flex items-center gap-2 min-w-0">
                      <span
                        className="text-sm text-gray-400 shrink-0 inline-flex items-center justify-center w-4 h-4"
                        aria-label={active ? t.tracks.playing : t.tracks.play}
                      >
                        {active ? (
